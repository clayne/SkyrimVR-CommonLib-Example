...
namespace HiggsPluginAPI {
    ...
    struct IHiggsInterface001 {
        ...
        // Grab the given object reference. It must have collision, and the given hand must be in a ready state as
        // returned by CanGrabObject.
        ...
        // (no currently held object, not pulling anything or have an object locked in for pulling (i.e. trigger/grip
        // held on a selected object)
        virtual bool IsHandInGrabbableState(bool isLeft) = 0;

        // Disable and enable grabbing, selecting, pulling, etc. for each hand. Every disable should be accompanied by a
        // later enable. Multiple mods can disable at once, and the hand is only re-enabled once all mods have called
        // enable.
        ...
        // This includes whether it is in a grabbable state, but also whether it is holding a blocking weapon or
        ...
        typedef CollisionFilterComparisonResult (*CollisionFilterComparisonCallback)(void *collisionFilter,
                                                                                     uint32_t filterInfoA,
                                                                                     uint32_t filterInfoB);
        ...
        // Get/set the collision layer bitfield for the higgs collision layer (used for hands, weapons, and held
        // objects).
        ...
        // Get the current amount that each finger is curled. Values range from 0 to 1, where 1 is fully open and 0 is
        // fully curled. Order is from thumb to pinky.
        ...
        // Read or modify any of higgs's numeric ini settings. Returns true if the option exists and is gotten/set, and
        // false otherwise. Only some settings will have an effect if modified, depending on if they are read at
        // startup, when loading / switching cells, or at the time that they are actually required.
        ...
        // Get/set the transform of the current grabbed object in the space of the hand that grabs it, i.e. the
        // hand-to-grabbed-node transform. This is the transform the grabbed node is driven/set to, as well as the
        // transform the hand is set from when in physics-grab mode.
